...
namespace CasosUso.Manejadores
{
    public class ManejadorPlanta : IManejadorPlanta
    {
        ...
        public ManejadorPlanta(IRepositorioPlanta repoPlan, IRepositorioTipoPlanta repoTipo, IRepositorioTipoIluminacion repoIlu)
        {
            RepoPlanta = repoPlan;
            RepoIluminacion = repoIlu;
            RepoTipo = repoTipo;
        }
        ...
        public bool AgregarPlanta(Planta planta, int idTipo, int idIluminacion, string listaNom)
        {
            ...
            {
                TipoIluminacion tipoIluminacion = RepoIluminacion.FindById(idIluminacion); 
                if (tipoIluminacion != null)
                {
                    TipoPlanta tipoPlanta = RepoTipo.FindById(idTipo);
                    if (tipoPlanta != null)
                    {
                        planta.FichaCuidados.Iluminacion = tipoIluminacion;
